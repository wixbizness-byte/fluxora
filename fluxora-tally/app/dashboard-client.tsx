...
export default function DashboardClient() {
  ...
  const theme = { "--fx-brand": current.settings.accentColor, "--fx-canvas": current.settings.backgroundColor, "--fx-surface": current.settings.cardColor, "--fx-text": current.settings.textColor } as CSSProperties;
  function createAccount() {
    setEditor({ mode: "new", account: { id: makeId("account"), name: "", url: "", statusId: current.statuses[0].id, values: Object.fromEntries(current.fields.map((field) => [field.id, ""])) } });
  }
  function persistAccount(account: TallyAccount, mode: "new" | "edit") {
    const accounts = mode === "new" ? [...current.accounts, account] : current.accounts.map((item) => item.id === account.id ? account : item);
    setEditor(null); void save({ ...current, accounts });
  }
  function openConfig() { setLogoError(""); setConfig({ settings: { ...current.settings }, fields: current.fields.map((f) => ({ ...f })), statuses: current.statuses.map((s) => ({ ...s })) }); }
  ...
  return <div className="app-shell" style={theme}>
    <header className="site-header"><div className="header-inner">
      <div className="brand-lockup"><div className="brand-image-wrap"><img className="brand-logo" src={current.settings.logoUrl || "/favicon.png"} alt="" /></div><div className="brand-copy"><h1>{current.settings.siteTitle}</h1><p>{current.settings.subtitle}</p></div></div>
    ...
    <main className="dashboard-main">
      ...
      <section className="summary-section" aria-labelledby="summary-title"><div className="section-heading"><div><p className="eyebrow">Overview</p><h2 id="summary-title">Summary totals</h2></div><p className="section-note">Totals add up the values you enter manually.</p></div><div className="summary-grid"><article className="summary-card"><span>Accounts</span><strong>{current.accounts.length}</strong></article>{current.fields.filter((field) => field.showInSummary).map((field) => <article className="summary-card" key={field.id}><span>{field.label}</span><strong>{formatValue(current, String(current.accounts.reduce((total, account) => total + (parseNumber(account.values[field.id]) ?? 0), 0)), field)}</strong></article>)}</div></section>

      <section className="accounts-section" aria-labelledby="accounts-title"><div className="section-heading accounts-heading"><div><p className="eyebrow">Accounts</p><h2 id="accounts-title">Your tally</h2></div><div className="toolbar"><label className="search-control"><span className="sr-only">Search accounts</span><SearchIcon size={17} /><input type="search" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search accounts" /></label><label className="sort-control"><span className="sr-only">Sort accounts</span><select value={current.sortMode} onChange={(e) => void save({ ...current, sortMode: e.target.value as SortMode })}><option value="manual">Manual order</option><option value="name-asc">Name A–Z</option><option value="name-desc">Name Z–A</option>{current.fields.flatMap((field) => [<option key={`${field.id}-desc`} value={`field:${field.id}:desc`}>{field.label}: high to low</option>, <option key={`${field.id}-asc`} value={`field:${field.id}:asc`}>{field.label}: low to high</option>])}</select></label><button className="button button-primary" onClick={createAccount}><PlusIcon size={16} /> Add account</button></div></div>
